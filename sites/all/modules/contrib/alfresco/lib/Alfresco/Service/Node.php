...
require_once 'Alfresco/Service/ChildAssociation.php';
require_once 'Alfresco/Service/Association.php';
...
class AlfNode extends AlfBaseObject {
   ...
   * Constructor
   */
  public function __construct($session, $store, $id, $score = 1.0) {
  ...
  public function updateContent($property, $mimetype, $encoding = "UTF-8", $content = null) {
    ...
    $contentData = new AlfContentData($this, $property, $mimetype, $encoding);
    if ($content != null) {
  ...
  public function createChild($type, $associationType, $associationName) {
    ...
    $newNode = new AlfNode($this->_session, $this->_store, $id);
    $childAssociation = new AlfChildAssociation($this, $newNode, $associationType, $associationName, true);
  ...
  public function addChild($node, $associationType, $associationName) {
    ...
    $childAssociation = new AlfChildAssociation($this, $node, $associationType, $associationName, false);
  ...
  public function addAssociation($to, $associationType) {
    ...
    $association = new AlfAssociation($this, $to, $associationType);
  ...
  public function createVersion($description = null, $major = false) {
    ...
    $result = $client->createVersion(array(
      "items" => array(
    ...
    return new AlfVersion($this->_session, new AlfStore($this->_session, $versionStoreAddress, $versionStoreScheme) , $versionId);
  }

  private function isDirty() {
    $result = true;
    if ($this->_isNewNode == false && count($this->getModifiedProperties()) == 0 && ($this->addedAspects == null || count($this->addedAspects) == 0) && ($this->removedAssociations == null || count($this->removedAssociations) == 0) && ($this->addedChildren == null || count($this->addedChildren) == 0) && ($this->addedAssociations == null || count($this->addedAssociations) == 0)) {
      $result = false;
    }
    return $result;
  }
  ...
  public function __set($name, $value) {
    ...
    if ($fullName != $name) {
      ...
      if ($value instanceof AlfContentData) {
  ...
  public function __toString() {
    return AlfNode::__toNodeRef($this->_store, $this->id);
  }
  ...
  public function getVersionHistory() {
    if ($this->_versionHistory == null) {
      $this->_versionHistory = new AlfVersionHistory($this);
    }
    ...
    foreach($webServiceNode->properties as $propertyDetails) {
      ...
      if ($isMultiValue == false) {
        ...
        if ($this->isContentData($value) == true) {
          $value = new AlfContentData($this, $name);
        }
      } else {
  ...
  private function populateChildren() {
    ...
    $result = $this->_session->repositoryService->queryChildren(array(
      "node" => $this->__toArray()
    ));
    ...
    foreach($map as $value) {
      ...
      $child = $this->_session->getNode(new AlfStore($this->_session, $store_address, $store_scheme) , $id);
      $children[$child->__toString()] = new AlfChildAssociation($this, $child, $assoc_type, $assoc_name, $isPrimary, $nthSibling);
    }
  ...
  private function populateAssociations() {
    ...
    foreach($map as $value) {
      ...
      $store_address = $value["{http://www.alfresco.org/model/system/1.0}store-identifier"];
      $assoc_type = $value["associationType"];
      $to = $this->_session->getNode(new AlfStore($this->_session, $store_address, $store_scheme) , $id);
      $associations[$to->__toString() ] = new AlfAssociation($this, $to, $assoc_type);
    }
  ...
  private function populateParents() {
    // TODO should do some sort of limited pull here
    $result = $this->_session->repositoryService->queryParents(array(
      "node" => $this->__toArray()
    ));
    $resultSet = $result->queryReturn->resultSet;
    $parents = array();
    ...
    foreach($map as $value) {
      ...
      $isPrimary = $value["isPrimary"];
      $nthSibling = $value["nthSibling"];
      $parent = $this->_session->getNode(new AlfStore($this->_session, $store_address, $store_scheme) , $id);
      if ($isPrimary == "true" or $isPrimary == true) {
        $this->_primaryParent = $parent;
      }
      $parents[$parent->__toString() ] = new AlfChildAssociation($parent, $this, $assoc_type, $assoc_name, $isPrimary, $nthSibling);
    }
  ...
  public function onBeforeSave(&$statements) {
    ...
    if ($this->_isNewNode == true) {
      ...
      $this->addStatement($statements, "create", array(
        "id" => $this->_id
      ) + $parentArray + array(
        "type" => $this->_type,
      ...
      if (count($modifiedProperties) != 0) {
        $this->addStatement($statements, "update", array(
          "property" => $this->getPropertyArray($modifiedProperties)
        ) + $this->getWhereArray());
      }
    ...
    if ($this->_properties != null) {
      foreach($this->_properties as $name => $value) {
        if (($value instanceof AlfContentData) && $value->isDirty == true) {
    ...
    if ($this->addedAspects != null) {
      foreach($this->addedAspects as $aspect) {
        $this->addStatement($statements, "addAspect", array(
          "aspect" => $aspect
        ) + $this->getWhereArray());
      }
    }
    ...
    if ($this->removedAspects != null) {
      foreach($this->removedAspects as $aspect) {
        $this->addStatement($statements, "removeAspect", array(
          "aspect" => $aspect
        ) + $this->getWhereArray());
      }
    }
  ...
  private function getPropertyArray($properties) {
    ...
    foreach($properties as $name => $value) {
      ...
      if (($value instanceof AlfContentData) == false) {
  ...
  /**
   * Get folder path for node
   * aw@abcona.de 2009-10-12
   */
  public function getFolderPath() {
    if (!$this->_folderPath) {
      ...
      while ($parentFolder = $currentFolder->getPrimaryParent()) {
